...
export const RadioSelect:FC<radioSelectInterface> = ({options, formInputs, label, isRequired, name, setFormInputs, flex }) => {
    return(
        <div className={`flex flex-col w-full gap-2 relative`}>
            ...
            <div className={`flex ${flex == 'row' ? "gap-9" : "flex-col"} gap-2`}>
                {
                    options.map((o, i) => (
                        <div key={i} className="flex items-center gap-2 cursor-pointer" onClick={() => {
                            ...
                            <div className={`size-5 transition-ll duration-1000 rounded-full ${formInputs[name] == o.replace(' ', '-').toLowerCase() ? "bg-primary border-0" : "bg-gray-100 border border-gray-400"}`}></div>
